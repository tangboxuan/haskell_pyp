...
tryToLookUp n y xs
  = fromMaybe y (lookup n xs)

-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp n xs
  = map fst (filter (\x -> (snd x) == n) xs)

occurs :: String -> Type -> Bool
occurs str (TVar x)
  = str == x
occurs str (TFun x y)
  = occurs str x || occurs str y
occurs _ _
  = False
...
applySub s (TFun a b)
  = TFun (applySub s a) (applySub s b)
applySub _ x
  = x
...
unifyPairs ((TBool, TBool):xs) s
  = unifyPairs xs s
-- Rule 2
unifyPairs ((TVar a, TVar b):xs) s
  | a == b    = unifyPairs xs s
  | otherwise = Nothing
unifyPairs ((TVar a, b):xs) s
  | occurs a b = Nothing
  | otherwise  = unifyPairs xs' ((a, b):s)
  where
    xs' = map (\(x, y) -> (f x, f y)) xs
    f = applySub [(a, b)]
unifyPairs ((a, TVar b):xs) s
  = unifyPairs ((TVar b, a):xs) s
-- Rule 3
