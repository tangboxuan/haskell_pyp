...
lookUpAtt n h r
  = lookUp n (zip (map fst h) r)

removeAtt :: AttName -> Header -> Row -> Row
removeAtt n h r
  = map snd (remove n (zip (map fst h) r))

addToMapping :: Eq a => (a, b) -> [(a, [b])] -> [(a, [b])]
addToMapping (x, v) []
  = [(x, [v])]
addToMapping (x, v) (m@(x', vs):ms)
  | x == x' = (x, v:vs):ms
  | otherwise = m : addToMapping (x, v) ms
...
buildFrequencyTable (n, vs) (h, rs)
  = [(v, count v) | v <- vs]
  where
    vs' = [lookUpAtt n h r | r <- rs]
    count v = length (filter (==v) vs')
...
partitionData (h, rs) (n, vs)
  = [(v, (h', partition' v)) | v <- vs]
  where
    partition' v
      = map (removeAtt n h) rs'
      where
        rs' = filter ((v ==) . lookUpAtt n h) rs
    h' = remove n h
...
buildTree d@(h, rs) c@(cn, _) f
  ...
  | otherwise  = Node an bs'
  where
    cs = [lookUpAtt cn h r | r <- rs]
    a@(an, _) = f d c
    bs = partitionData d a
    bs' = map (\(v, d) -> (v, buildTree d c f)) bs
