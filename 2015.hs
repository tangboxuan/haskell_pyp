...
getLocals s
  = filter (\(_, (x, _)) -> x == Local) s

getGlobals :: State -> State
getGlobals s
  = filter (\(_, (x, _)) -> x == Global) s
...
applyOp Equal (I x) (I y)
  | x == y    = I 1
  | otherwise = I 0
applyOp Index (A xs) (I n)
  = I (fromMaybe 0 (lookup n xs))

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs is vs
  = zipWith (\i v -> (i, (Local, v))) is vs
...
eval (Var v) _ s
  = getValue v s
eval (Cond p t f) fds s
  | eval p fds s == I 1 = eval t fds s
  | otherwise           = eval f fds s
eval (OpApp o x y) fds s
  = applyOp o (eval x fds s) (eval y fds s)
...
executeStatement (Assign i e) fds _ s
  = updateVar (i, eval e fds s) s
executeStatement (AssignA i n e) fds _ s
  = updateVar (i, a') s
  where
    a = getValue i s
    a' = assignArray a (eval' n) (eval' e)
    eval' e
      = eval e fds s
executeStatement (If e b b') fds pds s
  | eval e fds s == I 1 = executeBlock b fds pds s
  | otherwise           = executeBlock b' fds pds s
executeStatement sm@(While e b) fds pds s
  | eval e fds s == I 1 = executeStatement sm fds pds (executeBlock b fds pds s)
  | otherwise           = s
executeStatement (Call i p es) fds pds s
  | i == ""   = s'''
  | otherwise = updateVar (i, v) s'''
  where
    (as, b) = lookUp p pds
    vs = evalArgs es fds s
...
executeStatement (Return e) fds pds s
  = executeStatement (Assign "$res" e) fds pds s

executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock [] _ _ s
  = s
executeBlock (sm:sms) fds pds s
  = executeBlock sms fds pds (executeStatement sm fds pds s)
