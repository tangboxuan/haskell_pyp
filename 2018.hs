...
update p []
  = [p]
update p'@(id', _) (p@(id, _):ps)
  | id == id' = p' : ps
  | otherwise = p : update p' ps
...
execStatement sm@(DoWhile b e) s
  | eval e s' == 1 = execStatement sm s'
  | otherwise      = s'
  where
    s' = execBlock b s
...
execBlock (sm:sms) s
  = execBlock sms (execStatement sm s)
execBlock [] s
  = s
...
foldConst e@(Phi e1 e2)
  | e1 == e2  = e1
  | otherwise = e
foldConst (Apply o (Const x) (Const y))
  = Const (apply o x y)
foldConst (Apply Add v (Const 0))
  = v
foldConst (Apply Add (Const 0) v)
  = v
foldConst e
  = e
...
sub id v e@(Var id')
  | id == id' = Const v
  | otherwise = e
sub id v (Apply o e1 e2)
  = foldConst (Apply o (sub id v e1) (sub id v e2))
sub id v (Phi e1 e2)
  = foldConst (Phi (sub id v e1) (sub id v e2))
sub _ _ e
  = e

-- Use (by uncommenting) any of the following, as you see fit...
type Worklist = [(Id, Int)]
scan :: Worklist -> Block -> Block
scan wl b
  | null wl   = b
  | otherwise = scan rwl rb
  where
    b' = scan1 b
    ((id, v):ps) = wl
    (rwl, rb) = scan2 ps [] b'
    ...
    scan1 []
      = []
    scan1 ((Assign i e):sms)
      | id == i   = scan1 sms
      | otherwise = (Assign i (sub' e)) : scan1 sms
    scan1 ((If e b1 b2):sms)
      = (If (sub' e) (scan1 b1) (scan1 b2)): scan1 sms
    scan1 ((DoWhile b e):sms)
      = (DoWhile (scan1 b)) (sub' e): scan1 sms

    scan2 wl nb [] 
      = (wl, nb)
    scan2 wl nb (sm@(Assign "$return" _):sms)
      = scan2 wl (nb++[sm]) sms
    scan2 wl nb ((Assign i (Const c)):sms)
      = scan2 ((i, c):wl) nb sms
    scan2 wl nb ((If e b1 b2):sms)
      = scan2 (wl++wl1++wl2) (nb++[(If e nb1 nb2)]) sms
      where
        (wl1, nb1) = scan2 [] [] b1
        (wl2, nb2) = scan2 [] [] b2
    scan2 wl nb ((DoWhile b e):sms)
      = scan2 (wl++wl1) (nb++[(DoWhile nb1 e)]) sms
      where
        (wl1, nb1) = scan2 [] [] b
    scan2 wl nb (sm:sms)
      = scan2 wl (nb++[sm]) sms
