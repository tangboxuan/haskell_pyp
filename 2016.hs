...
getValue (Element _ _ cs)
  = Text (concat (map (getText . getValue) cs))
  where
    getText (Text str)
      = str
...
-- expandXSL' c (Element n a xs)
--   | n == "value-of" = [Text p]
--   | otherwise = [Element n a (concatMap (expandXSL' c) xs)]
--   where
--     p = fromJust (lookup "select" a)
-- expandXSL' _ x
--   = [x]
