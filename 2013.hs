...
-- Pre: b1 and b2 have same rank
combineTrees b1@(Node v1 r c1) b2@(Node v2 _ c2)
  | v1 < v2   = Node v1 (r+1) (b2:c1)
  | otherwise = Node v2 (r+1) (b1:c2)
...
deleteMin b
  = mergeHeaps (reverse h) h'
  where
    ((Node _ _ h), h') = removeMin b


remove :: Eq a => a -> BinHeap a -> BinHeap a
-- Pre: Value x occurs in the BinHeap
remove x (b:bs)
  | x == value b = bs
  | otherwise    = b : remove x bs

removeMin :: Ord a => BinHeap a -> (BinTree a, BinHeap a)
removeMin b
  = (head (filter ((==m) . value) b), remove m b)
  where
    m = extractMin b

binSort :: Ord a => [a] -> [a]
binSort xs
  = deconstruct (construct xs)
  where
    construct []
      = []
    construct (x:xs)
      = insert x (construct xs)
    deconstruct []
      = []
    deconstruct xs
      = extractMin xs : deconstruct (deleteMin xs)
